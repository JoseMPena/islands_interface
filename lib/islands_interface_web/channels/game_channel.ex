defmodule IslandsInterfaceWeb.GameChannel do
  ...
  def handle_in("new_game", _payload, socket) do
    ...
    response =
      case GameSupervisor.start_game(player) do
        {:ok, _pid} -> :ok
        {:error, reason} -> %{reason: inspect(reason)}
      end
  ...
  def handle_in("set_islands", player, socket) do
    ...
    response =
      case Game.set_island(via(socket.topic), player) do
        {:ok, board} ->
          broadcast!(socket, "player_ser_islands", %{player: player})
          {:ok, %{board: board}}
  ...
  def handle_in("guess_coordinate", params, socket) do
    ...
    case Game.guess_coordinate(via(socket.topic), player, row, col) do
      ...
      :error ->
        {:reply, {:error, %{player: player, reason: "No es tu turno."}}, socket}
  ...
  # Fetch via tuple for a specific game
